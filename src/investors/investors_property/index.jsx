...
export default function Investors_property() {
    ...
    const getInvestors = () => {
        axios.get(`${process.env.REACT_APP_NEXT_PUBLIC_BASE_URL}/get-investors-warehouses/?company_type=${companyType}`,
            {
                headers: {
            ...
            .then((res) => {
                // console.log("investors property res", res.data.response)
                if (res) {
            ...
            .catch((err) => {
                console.log("investors property err", err)
                setLoader(false)
            })
    }
    useEffect(() => {
    ...
    const handleSearch = (e) => {
        ...
        if (e.target.value.length > 0) {
            const searchData = investors.filter((val) => val.property_name.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase())
    ...
    const downloadPdf = () => {
        ...
        const doc = new jsPDF()
        doc.text("Investor Property Details", 20, 10)
        doc.autoTable({
            head: [headerNames],
            body: pdfData,
        })
        doc.save('investor_property.pdf')
    }
    //******** download pdf*********//
    return (
                    ...
                    <Grid item lg={12} xs={12}>
                        <Box sx={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", mb: 2, px: "1em", pb: "1em", borderBottom: "1px solid #8080802e" }}>
                            ...
                            <ButtonGroup variant="contained" aria-label="outlined secondary button group" className='buttonGroup'>
                                ...
                                <Button><CSVLink data={fileData} filename={"investors_property_list.csv"}><FontAwesomeIcon icon={faFileCsv} />&nbsp; CSV</CSVLink></Button>
                                <Button><FontAwesomeIcon icon={faFileExcel} />&nbsp; <ExportToExcel details={fileData} fileName="Investor Property List" /></Button>
                            </ButtonGroup>
                        </Box>
                                                    ...
                                                    return (
                                                        <TableRow key={i}>
                                                            ...
                                                            {/* <TableCell component="th" scope="row">
                                                                {e.total_number_of_investors}
                                                            </TableCell> */}
                                                            <TableCell component="th" scope="row">
                                                                {
                                                                    // e.total_number_of_galas === 0 ? "-" :
                                                                    <Link to={`/investors/investors_property/${e.uid}`} style={{ textDecoration: "none" }}>
