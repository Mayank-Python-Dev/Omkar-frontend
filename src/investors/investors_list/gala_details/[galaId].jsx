...
export default function InvestorGalaDetails() {
    const navigate = useNavigate()
    const path = useParams()
    const galaId = path.slug
    const [galaData, setGalaData] = useState([])
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - galaData.length) : 0;
    ...
    const getGalas = () => {
        axios.get(`${process.env.REACT_APP_NEXT_PUBLIC_BASE_URL}/get-investor-remaining-gala-detail/${galaId}/?company_type=${companyType}`,
            {
                headers: {
                            ...
                            <Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <StyledTableCell>Gala Price</StyledTableCell>
                                    </TableRow>
                                </TableHead>
                                ...
                                <TableBody>
                                    {
                                        loader ?
                                            ...
                                            galaData.length > 0 ?
                                                (rowsPerPage > 0
                                                    ? galaData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                                                    : galaData
                                                ).map((e, i) => {
                                                    return (
                                                        <TableRow key={i}>
