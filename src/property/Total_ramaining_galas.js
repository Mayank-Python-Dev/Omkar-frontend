...
export default function Total_ramaining_galas() {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [remainingGala, setRemainingGala] = useState([])
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - remainingGala.length) : 0;
    ...
    const getRemainingGala = async () => {
        await axios.get(`${process.env.REACT_APP_NEXT_PUBLIC_BASE_URL}/get-total-remaining-galas/?company_type=${companyType}`,
            {
                headers: {
            ...
            .then((res) => {
                console.log("remainingGala res", res.data.response)
                if (res) {
                    setRemainingGala(res.data.response)
                    ...
                    for (let i = 0; i < res.data.response.length; i++) {
                        ...
                        obj['Owner'] = res.data.response[i].warehouse.farmer_warehouse_detail !== null ? res.data.response[i].warehouse.farmer_warehouse_detail.user.username : "Developer"
                        setFileData((a) => [...a, obj])
                    }
                    for (let i = 0; i < res.data.response.length; i++) {
                        let arr = []
                        ...
                        arr[6] = res.data.response[i].warehouse.farmer_warehouse_detail !== null ? res.data.response[i].warehouse.farmer_warehouse_detail.user.username : "Developer"
            ...
            .catch((err) => {
                console.log("remainingGala err", err)
                setLoader(false)
            })
    }
    useEffect(() => {
    ...
    const handleSearch = (e) => {
        ...
        if (e.target.value.length > 0) {
            const searchData = remainingGala.filter((val) => val.gala_number.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase())
                || val.warehouse.property_name.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase())
            )
            setRemainingGala(searchData)
        } else {
            setRemainingGala(filterData)
        }
    }
    // ****** search filter end******
    ...
    const downloadPdf = () => {
        ...
        doc.text("Remaining Gala Details", 20, 10)
        doc.autoTable({
            head: [headerNames],
            body: pdfData,
        })
        doc.save('remaining gala.pdf')
    }
    //******** download pdf*********//
                ...
                <Paper variant="outlined" sx={{
                    ...
                    <Grid item lg={12} xs={12}>
                        <Box sx={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", mb: 2, px: "1em", pb: "1em", borderBottom: "1px solid #8080802e" }}>
                            <Typography variant="subtitle1" sx={{ fontSize: "20px" }} gutterBottom>Remaining Gala List</Typography>
                            <ButtonGroup variant="contained" aria-label="outlined secondary button group" className='buttonGroup'>
                                ...
                                <Button><CSVLink data={fileData} filename={"remainingGala.csv"}><FontAwesomeIcon icon={faFileCsv} />&nbsp; CSV</CSVLink></Button>
                                <Button><FontAwesomeIcon icon={faFileExcel} />&nbsp; <ExportToExcel details={fileData} fileName="remainingGala" /></Button>
                            </ButtonGroup>
                        </Box>
                                ...
                                <TableBody>
                                    {
                                        loader ?
                                            ...
                                            :
                                            remainingGala.length > 0 ?
                                                (rowsPerPage > 0
                                                    ? remainingGala.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                                                    : remainingGala
                                                ).map((e, i) => {
                                                    return (
                                                        <TableRow key={i} hover>
                                                            ...
                                                            <TableCell component="th" scope="row">
                                                                {e.warehouse.farmer_warehouse_detail !== null ? <span style={{ textTransform: "capitalize" }}>{e.warehouse.farmer_warehouse_detail.user.username}</span> : "Developer"}
                                                            </TableCell>
                                                        </TableRow>
                                                    )
                                                })
